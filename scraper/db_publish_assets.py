...
def release_notes(man: dict) -> str:
    ...
    return (
        "Public U.S. sex offender registry SQLite archive for Arrest Public Archiver.\n\n"
        ...
        "- Clients apply `arrests.delta.NNNN.zip` after the base for small updates\n"
        "- Paths are project-relative under `data/photos/*/photos/`\n"
    )
...
def select_upload_assets(
    *,
    ...
    remote_has_base = "arrests.db.zip" in remote_names
    if zip_path.is_file() and (full_base or not remote_has_base):
        assets.insert(0, zip_path)
    for p in delta_paths:
        if p.is_file():
            assets.append(p)
    remote_photos = {n for n in remote_names if n.startswith("arrests.photos.")}
    if not skip_photos:
