...
def load_deepface_hits_as_misclass(
    ...
    *recorded_races* / *face_labels* default to the Scan tab settings
    ...
    try:
        ...
        loader = (
            db.list_deepface_scored
            if revalidate
            else db.list_deepface_hits
        )
        rows = loader(
            limit=limit,
...
def scan_gross_misclassifications(
    ...
    *,
    ...
    the recorded race (default: Black / Indian / Asian face vs race=White).

    Does **not** use surname lists — pure vision filter for gross errors.

    *skip_scanned* / *force_rescan*: by default, arrests already stored in
    ...
    *on_scored*: optional callback ``(record, face, is_hit, index, total)``
    ...
    # Collect candidates with photos (arrests table)
    sql = (
        "SELECT * FROM arrests "
