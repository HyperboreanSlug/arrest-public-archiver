...
_SUMMARY_RULES_A: List[Tuple[str, List[str]]] = [
    ...
    (
        "DUI / DWI",
        [r"\bdui\b", r"\bdwi\b", r"\bowi\b", r"\bovi\b", r"\bd\.?\s*u\.?\s*i\.?\b", r"\bdriving\s+under\s+the\s+inf", r"\bdriving\s+while\s+(intoxic|impair)", r"\bimpaired\s+driving", r"\baggravated\s+dui", r"\b32-5a-191\b", r"\boperating\s+.*under\s+influence", r"\bblood\s+alcohol", r"\bbac\b"],
    ),
    (
        "RECKLESS DRIVING",
        [r"\breckless\s+driv"],
    ...
    (
        ...
        [r"\bdriv(ing)?\s+w(ith)?\s*(lic|license).*(susp|revok)", r"\bdriv(ing)?\s+while\s+(license|driver).*(susp|revok)", r"\balias\s+driving\s+w\s+(susp|revok)", r"\bsuspended\s+license", r"\brevoked\s+license", r"\bno\s+license\b", r"\bwithout\s+ever\s+obtaining\s+license", r"\boperating\s+without\s+.*license"],
    ),
    (
        "ELUDING / FLEEING",
        [r"\belud", r"\bfleeing\b", r"\battempting\s+to\s+elude", r"\batepo\b"],
    ),
    (
        "TRAFFIC OFFENSE",
    ...
    (
        ...
        [r"\bchild\s+abuse", r"\bchild\s+endanger", r"\bneglect\s+of\s+child"],
    ),
    # --- Sex crimes ---
    (
        "SEX OFFENSE",
