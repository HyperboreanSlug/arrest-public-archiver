...
def gh_prune_obsolete_photos(repo: str, tag: str, keep: Set[str]) -> None:
    ...
    photo_keep = {n for n in keep if n.startswith("arrests.photos.")}
    if not photo_keep:
        return
    try:
        cp = subprocess.run(
    ...
    for asset in data.get("assets") or []:
        ...
        if not name.startswith("arrests.photos.") or name in photo_keep:
            continue
        aid = asset.get("id")
        if not aid:
            continue
...
def publish_api(
    ...
    *,
    ...
    photo_keep = {n for n in asset_names if n.startswith("arrests.photos.")}
    for asset in rel.get("assets") or []:
        ...
        if (
            prune_photos
            and photo_keep
            and name.startswith("arrests.photos.")
