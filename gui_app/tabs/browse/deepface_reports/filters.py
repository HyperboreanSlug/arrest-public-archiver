"""Browse → DeepFace Reports: filter, tree, metrics, verdict readers."""
from __future__ import annotations

from gui_app.widgets import _format_race_display, _format_state_display


class DeepfaceReportsFiltersMixin:
    """Verdict filter keys, tree population, metrics."""
    ...
    def _dfr_apply_filters(self) -> None:
        ...
        for mc in all_hits:
            ...
            if face_f and face_f != "all":
                ...
                lab = lab.replace(" ", "_").replace("(south_asian)", "").replace(
                    "indian_(south_asian)", "indian"
                )
                if "indian" in lab:
                    lab = "indian"
