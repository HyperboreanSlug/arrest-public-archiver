"""Auto-confirm misclassification when a share card is exported (MAPA)."""
...
def mark_export_confirmed_incorrect(
    ...
    *,
    ...
    """Mark this person confirmed *incorrect* after card export.

    Writes ``ethnicity_review=incorrect`` into ``arrests.flags`` (and identity
    siblings via persist_ethnicity_verdict) and ``report_verdicts.json``.
    """
    ...
    if not path:
        try:
            ...
            path = str(project_root() / "data" / "arrests.db")
        except Exception:
            path = "data/arrests.db"
