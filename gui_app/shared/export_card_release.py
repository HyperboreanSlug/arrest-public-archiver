"""Persistent card export numbers for MAPA export cards.
...
Numbers are stored in:
  - ``arrests.export_number`` (authoritative once assigned)
...
def _persist_to_db(record: Mapping[str, Any], num: int) -> None:
    ...
    try:
        ...
        if raw_path:
            ...
            db_path = str(project_root() / "data" / "arrests.db")
        try:
            rid_i = int(rid)
        except (TypeError, ValueError):
            return
...
def release_number_for(
    ...
    *,
    ...
    ``arrests.export_number`` when ``id`` is present.
    """
    key = person_card_key(record)
    store = Path(path) if path is not None else _store_path()
    existing = export_number_on_record(record)
