...
def fit_image_to_box(img: Any, box: Tuple[int, int]) -> Any:
    ...
    # thumbnail mutates in place and never exceeds box
    out = out.copy()
    out.thumbnail((max_w, max_h), resample)
    # Guard: never larger than box even if thumbnail edge cases
    ...
    def work() -> None:
        ...
        def apply() -> None:
            ...
            if pil_fit is None:
                ...
                return
            try:
                # CTkImage size = fitted pixel size (always ≤ box)
                disp = (int(pil_fit.width), int(pil_fit.height))
                image = ctk.CTkImage(
                    light_image=pil_fit, dark_image=pil_fit, size=disp
                )
                set_photo_fn(image)
            except Exception as exc:
                set_photo_fn(
...
def render_fitted_ctk_image(
    pil_source: Any, box: Tuple[int, int]
) -> Optional[Any]:
    """Sync: fit an already-loaded PIL image into *box* as a CTkImage."""
    if pil_source is None:
        return None
    try:
        fitted = fit_image_to_box(pil_source, box)
        disp = (int(fitted.width), int(fitted.height))
        return ctk.CTkImage(light_image=fitted, dark_image=fitted, size=disp)
    except Exception:
        return None
